...
class AwsAutoScalingService implements CacheInitializer, InitializingBean {
	...
	/**
	 * Attach an EC2 instance to an existing AutoScalingGroup
	 * 
	 * @param userContext
	 * @param groupName the name of the AutoScalingGroup
	 * @param instanceId the instance we want to attach to the group
	 * @return the new AutoScalingGroup object after attaching instance
	 */
	AutoScalingGroup attachInstanceToAutoScalingGroup(UserContext userContext, String groupName, String instanceId) {
		Check.notNull(groupName, AutoScalingGroup, 'groupName')
		Check.notNull(instanceId, Instance, 'instanceId')
		AutoScalingGroup group = getAutoScalingGroup(userContext, groupName)
		if (group) {
			String msg = "Attach instance '${instanceId}' to auto scaling group '${groupName}'"
			
			taskService.runTask(userContext, msg, { Task task ->
				awsClient.by(userContext.region).attachInstances(
					new AttachInstancesRequest().withInstanceIds(instanceId).
	...
	/**
	 * Check whether an instance belongs to the specified AutoScalingGroup
	 * 
	 * @param userContext
	 * @param groupName
	 * @param instanceId
	 * @return true if the instance belongs to the ASG, false otherwise
	 */
	Boolean isInstanceWithinAutoScalingGroup(UserContext userContext, String groupName, String instanceId) {
		Check.notNull(groupName, AutoScalingGroup, 'groupName')
		Check.notNull(instanceId, Instance, 'instanceId')
		AutoScalingGroup group = getAutoScalingGroup(userContext, groupName)
		if (group) {
