...
class PushService {
	 ...
	 */
	RollingPushOperation suspendRollingPush(String rollingTaskId) {
		RollingPushOperation operation = null
		for(RollingPushOperation it : rollingPushes) {
			if(it.getTaskId().equals(rollingTaskId)) {
				operation = it
				break
			}
		}
		if(operation == null) {
			throw new Exception("This task is currently not running.")
		} else if(operation.task.status == 'suspended') {
			throw new Exception("This task is already being suspended.")
		}
		operation.suspend()
		operation
	}
	 ...
	 */
	RollingPushOperation resumeRollingPush(String rollingTaskId) {
		RollingPushOperation operation = null
		for(RollingPushOperation it : rollingPushes) {
			if(it.getTaskId().equals(rollingTaskId)) {
				operation = it
		...
		if(operation == null) {
			throw new Exception("This task is currently not running.")
		} else if(operation.task.status == 'running') {
			throw new Exception("This task is already running normally.")
		}
		operation.resume()
		operation
	}
