    ...
    <g:if test="${!task.isDone()}">
      <div class="buttons" id="taskCancellationForm">
        <g:form method="post">
          ...
          <g:if test="${task.name.matches("Pushing.*")}">
            <g:if test="${task.status.equals("running")}">
              <g:buttonSubmit class="pause" data-warning="Do you want to suspend execution of task ${StringEscapeUtils.escapeJavaScript(task.name)} ?" action="suspend" value="Pause Task"/>
            </g:if>
            <g:elseif test="${task.status.equals("suspended")}">
              <g:buttonSubmit class="resume" data-warning="Do you want to resume execution of task ${StringEscapeUtils.escapeJavaScript(task.name)} ?" action="resume" value="Resume Task"/>
            </g:elseif>
          </g:if>
        </g:form>
