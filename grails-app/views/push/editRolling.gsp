    ...
    <g:form method="post" class="validate">
      ...
      <div class="dialog">
        <table>
          ...
          <tr class="prop">
            <td colspan="100%"><h2 title="Options that specify the execution mode of Rolling Push">Rolling Push Execution Mode</h2></td>
          </tr>
          <tr class="prop" title="Choose whether to execute Rolling Push by continously relaunching instances (Default mode) or execute Rolling Push by waves (Synchronous mode)">
            <td class="name"><label id="executionModeLabel">Execution Mode:</label></td>
            <td class="value"><g:radio id="defaultMode" name="executionMode" value="default" checked="checked"/> <label for="defaultMode" class="choice">Default Mode: Continuously relaunch old instances based on the number of concurrent relaunches</label><br />
              <g:radio id="synchronousMode" name="executionMode" value="synchronous"/> <label for="synchronousMode" class="choice">Synchronous Mode: Execute Rolling Push by "waves", each wave will relaunch instances based on the number of concurrent relaunches. Must wait for a "wave" to finish before continuing to another wave</label>
            </td>
          </tr>
          <tr class="prop">
            <td colspan="100%"><h2 title="Options that specify the way Asgard handle unexpected instance failures during Rolling Push">Instance Failure Handling Mode</h2></td>
          </tr>
          <tr class="prop" title="Choose whether Asgard will replace unexpected failed instances during RollingPush operation by an instance with the new LaunchConfiguration (Default Mode), or by an instance with the old LaunchConfiguration first then do RollingPush on that instance (Revert Mode)">
            <td class="name"><label id="failureHandlingModeLabel">Instance Failure Handling Mode:</label></td>
            <td class="value"><g:radio id="defaultMode" name="failureHandlingMode" value="default" checked="checked"/> <label for="defaultMode" class="choice">Default Mode: When an instance unexpectedly fails during RollingPush, it will be replaced by another instance with the NEW Launch Configuration</label><br />
              <g:radio id="revertMode" name="failureHandlingMode" value="revert"/> <label for="revertMode" class="choice">Revert Mode: When an instance unexpectedly fails during RollingPush, it will be replaced by an instance with the OLD Launch Configuration first; then that replacement will undergo RollingPush to become a new instance with new LC</label>
            </td>
          </tr>
