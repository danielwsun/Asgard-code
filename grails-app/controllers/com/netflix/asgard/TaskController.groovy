...
class TaskController {
    ...
    def list = {
        ...
        List<Task> completed = completedTasks.sort { it.updateTime }.reverse().take(500)
	...
	/**
	 * TODO: currently only work with suspending Rolling Push tasks
	 * Hard-coded to redirect directly to PushController
	 */
	def suspend = {
		String id = params.id
		UserContext userContext = UserContext.of(request)
		...
		if (!task) {
			...
			redirect(controller: 'push', action: 'suspendRolling', params: [id: task.id])
		}
	}
	
	/**
	 * TODO: currently only work with resuming Rolling Push tasks
	 * Hard-coded to redirect directly to PushController
	 */
	def resume = {
		String id = params.id
		...
		if (!task) {
			...
			redirect(controller: 'push', action: 'resumeRolling', params: [id: task.id])
		}
	}
